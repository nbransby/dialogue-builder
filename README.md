...
* [it passes the supplied context to the script method](/tests.ts#177)
* [it throws an exception on empty script given](/tests.ts#193)
* [it throws an exception on script only containing labels](/tests.ts#215)
* [it sends the first and only message in a single message dialogue](/tests.ts#243)
* [it return no messages on consume when complete](/tests.ts#263)
* [it sends muliple say or ask messages at once](/tests.ts#285)
* [it trims extranous whitespace](/tests.ts#311)
* [it throws an exception on script with duplicate expect statements](/tests.ts#327)
* [it throws an exception on expect statement not followed by a response handler](/tests.ts#359)
* [it throws an exception on a response handler not preceeded by an expect statement](/tests.ts#403)
* [it pauses on expect to wait for a response](/tests.ts#433)
* [it resumes where it paused on recieving a response](/tests.ts#467)
* [it attaches any quick replies defined in response handler to last message](/tests.ts#493)
* [it attaches location quick reply if defined in response handler](/tests.ts#529)
* [it invokes a quick reply's handler on recieving the reply](/tests.ts#557)
* [it supports empty handlers](/tests.ts#579)
* [it supports null handlers](/tests.ts#605)
* [it throws an error when both location and onLocation specified on a handler](/tests.ts#655)
* [it prefers a quick reply handler to the onText handler](/tests.ts#689)
* [it invokes the location handler on recieving a location quick reply](/tests.ts#711)
* [it invokes the onText handler on recieving a text response](/tests.ts#735)
* [it invokes the onLocation handler on recieving a location response](/tests.ts#759)
* [it invokes the onImage handler on recieving an image response](/tests.ts#783)
* [it invokes the onVideo handler on recieving an video response](/tests.ts#807)
* [it invokes the onAudio handler on recieving an audio response](/tests.ts#831)
* [it invokes the onFile handler on recieving an file response](/tests.ts#855)
* [it handles unexpected response types by repeating only the ask statements](/tests.ts#879)
* [it does not send labels as messages](/tests.ts#915)
* [it respects inline gotos](/tests.ts#947)
* [it respects gotos retuned from response handlers](/tests.ts#983)
* [it throws an exception on calling goto with a missing label](/tests.ts#1021)
* [it throws an exception on script with duplicate labels](/tests.ts#1053)
* [it aborts a goto that causes an endless loop](/tests.ts#1085)
* [it resumes from the correct line when a goto skips a response handler](/tests.ts#1117)
* [it resets the dialogue when user sends a restart keyword](/tests.ts#1165)
* [it returns to previously asked question when user sends a undo keyword](/tests.ts#1205)
* [it supports a user sending an undo or restart keyword at the start of a dialogue](/tests.ts#1263)
